"""
[실험 A] LLM 필터링 + Calc_tool 계산기 방식

LLM은 "어떤 카드를 계산할지" 판단만 하고,
실제 혜택 금액 계산은 BenefitCalculator(코드)가 수행합니다.

흐름: 유저 입력 → LLM 필터링 → 스키마 변환 → Calc_tool 계산 → LLM 설명
...
os.environ["LANGSMITH_PROJECT"] = "Smart_Pick_Exp_A"
...
# ===========================< Test Log >============================
TEST_LOG_DIR = Path(__file__).resolve().parents[3] / "test_logs" / "exp_a"
PROMPT_VERSIONS = {"LLM_FILTER": "V1", "CALC": "Calc_tool", "EXPLAIN_PROMPT": "V3", "QA_PROMPT": "V1"}
...
def _init_test_log(total_budget: int, category_spending: dict):
    ...
    _test_log = {
        ...
        "experiment": "llm_filter_calc_tool",
        "input": {
...
# ===========================< Schema Adapter >============================
# json_v2 (간단한 형식) → Master Schema v2 (Calc_tool이 기대하는 형식) 변환
...
@observe(name="exp_a_llm_filter")
@traceable(run_type="chain", name="exp_a_llm_filter")
def llm_filter_node(state: AgentState):
    """LLM이 직접 카드를 필터링하고 Top 7을 선정합니다."""
    _node_start = time.time()
    print("[EXP-A] llm_filter_node")
...
@observe(name="exp_a_calc_tool")
@traceable(run_type="chain", name="exp_a_calc_tool")
def calc_tool_node(state: AgentState):
    """BenefitCalculator(코드)로 정확한 혜택 금액을 계산합니다."""
    _node_start = time.time()
    print("[EXP-A] calc_tool_node (BenefitCalculator)")
    ...
    for card in shortlist:
        # json_v2 → Master Schema v2 변환
        master_data = adapt_to_master_schema(card)

        # BenefitCalculator 실행
        calculator = BenefitCalculator(master_data)
    ...
    print(f"  [TIME] calc_tool: {_calc_elapsed}초")

    return {"calc_results": calc_results}


@observe(name="exp_a_rank_and_explain")
def rank_and_explain_node(state: AgentState):
    """최종 랭킹 후 LLM 추천 설명을 생성합니다."""
    _node_start = time.time()
    print("[EXP-A] rank_and_explain_node")
    ...
    # 실적 충족 카드만 필터 → 혜택 금액 기준 Top 3
    met_cards = [c for c in calc_results if c.get("performance_met", True)]
    if not met_cards:
        met_cards = calc_results  # 전부 미달이면 그냥 전체에서 선정
    ...
    # calc_summary (카테고리 breakdown 활용)
    breakdown_lines = []
...
@observe(name="exp_a_answer_qa")
def answer_qa_node(state: AgentState):
    """후속 질문 답변."""
    print("[EXP-A] answer_qa_node")
...
workflow.add_edge("calc_tool", "rank_and_explain")
...
if __name__ == "__main__":
    ...
    def run_test(case_id: str):
        ...
        session_id = f"exp_a_{uuid.uuid4().hex[:6]}"
    ...
    # 인터랙티브 메뉴
    while True:
        print("\n[실험A: LLM필터 + Calc_tool] 테스트 케이스를 선택하세요:")
