"""
[실험 C] LLM 계산 계획 생성 + 코드 전략 엔진 실행 방식

LLM은 카드/혜택 데이터를 보고 "어떤 전략을 어떤 순서로 실행할지" JSON 계획을 생성하고,
코드 엔진이 해당 계획에 명시된 전략만 선택적으로 실행합니다.

흐름: 유저 입력 → LLM 필터링 → 스키마 변환 → LLM 실행 계획 생성 → 코드 전략 엔진 → LLM 설명
...
os.environ["LANGSMITH_PROJECT"] = "Smart_Pick_Exp_C"
...
# ===========================< Test Log >============================
TEST_LOG_DIR = Path(__file__).resolve().parents[3] / "test_logs" / "exp_c"
PROMPT_VERSIONS = {"LLM_FILTER": "V1", "LLM_PLAN": "V1", "ENGINE": "StrategyEngine_V1", "EXPLAIN_PROMPT": "V3", "QA_PROMPT": "V1"}
...
def _init_test_log(total_budget: int, category_spending: dict):
    ...
    _test_log = {
        ...
        "experiment": "llm_plan_strategy_engine",
        "input": {
...
카드의 혜택 구조를 분석하고, 정확한 계산을 위해 어떤 전략을 어떤 순서로 실행해야 하는지 JSON 계획을 생성한다.
...
- TIER_SELECTION: 전월실적 구간별 혜택 매칭
- RATE_CALC: 정률 할인/적립 계산 (소비금액 × 할인율)
- FIXED_CALC: 정액 할인 계산 (건당 고정 금액)
- PER_UNIT_CALC: 단위당 할인 계산 (리터당 할인 등)
- TRANSACTION_LIMIT_APPLY: 거래 조건 적용 (최소결제, 일/월 횟수 제한)
- MONTHLY_LIMIT_APPLY: 월 한도 적용
- FALLBACK_APPLY: 한도 초과 시 기�� 적립률 적용
- GROUP_LIMIT_APPLY: 그룹 통합 한도 적용 (여러 혜택 공유 한도)
- WATERFALL_APPLY: 카테고리별 소비 차감 후 잔여액으로 전체 혜택 적용
- ANNUAL_SPECIALS: 연간 캐시백/바우처 혜택 산출
- ALL_DOMESTIC_SWEEP: 전 가맹점(All_Domestic) 혜택을 잔여 소비액에 적용

[작업]
위 카드의 혜택 구조를 분석하고, 유저의 소비 패턴에 맞는 최적의 실행 계획을 JSON으로 생성하라.

[분석 규칙]
1. pipeline: 카드 전체에 적용할 전략 (순서대로 실행)
   - 전월실적 조건이 있으면 PERFORMANCE_CHECK 필수
   - 실적 구간별 혜택이 있으면 TIER_SELECTION 추가
   - 혜택이 여러 개면 BENEFIT_LOOP 추가
   - 그룹 한도가 있으면 GROUP_LIMIT 추가
   - All_Domestic 혜택이 있고 다른 카테고리 혜택도 있으면 WATERFALL_APPLY 추가
   - 연간 특수 혜택이 있으면 ANNUAL_SPECIALS 추가
2. benefit_strategies: 각 혜택(benefit_id)별로 적용할 전략 리스트
   - calc_method가 RATE이면 RATE_CALC
   - calc_method가 FIXED_AMOUNT이면 FIXED_CALC
   - calc_method가 PER_UNIT이면 PER_UNIT_CALC
   - monthly_limit이 있으면 MONTHLY_LIMIT_APPLY 추가
   - fallback_reward_rate > 0이면 FALLBACK_APPLY 추가
   - 거래 조건(최소금액, 횟수제한 등)이 있으면 TRANSACTION_LIMIT_APPLY 추가
   - 유저 소비 카테고리와 무관한 혜택의 benefit_id는 포함하지 말 것
...
@observe(name="exp_c_llm_filter")
@traceable(run_type="chain", name="exp_c_llm_filter")
def llm_filter_node(state: AgentState):
    """LLM이 직접 카드를 필터링하고 Top 7을 선정합니다."""
    _node_start = time.time()
    print("[EXP-C] llm_filter_node")
...
@observe(name="exp_c_llm_plan")
@traceable(run_type="chain", name="exp_c_llm_plan")
def llm_plan_node(state: AgentState):
    """LLM이 각 카드별 실행 계획(JSON)을 생성합니다."""
    _node_start = time.time()
    print("[EXP-C] llm_plan_node")
    ...
    for card in shortlist:
        ...
        for b in master_data["benefits"]:
            calc_rule = b.get("calculation_rule", {})
            trans_cond = b.get("transaction_conditions", {})
            line = (
                f"  - {b['benefit_id']}: {b['category']} | "
                f"calc_method={calc_rule.get('calc_method')} | "
                f"rate={calc_rule.get('rate')} | "
                f"fixed={calc_rule.get('fixed_amount')} | "
                f"unit={calc_rule.get('unit_amount')} | "
                f"monthly_limit={calc_rule.get('monthly_limit')} | "
                f"fallback={calc_rule.get('fallback_reward_rate')} | "
                f"group_id={b.get('group_id')} | "
                f"content: {b.get('content', '')}"
            )
            benefits_lines.append(line)

        prompt = LLM_PLAN_PROMPT_V1.format(
            total_budget=total_budget,
            spending_lines=spending_lines,
            card_name=card.get("card_name", ""),
            card_company=card.get("card_company", ""),
...
@observe(name="exp_c_strategy_engine")
@traceable(run_type="chain", name="exp_c_strategy_engine")
def strategy_engine_node(state: AgentState):
    """LLM 계획에 따라 StrategyEngine이 선택적으로 전략을 실행합니다."""
    _node_start = time.time()
    print("[EXP-C] strategy_engine_node")
...
@observe(name="exp_c_rank_and_explain")
def rank_and_explain_node(state: AgentState):
    """최종 랭킹 후 LLM 추천 설명을 생성합니다."""
    _node_start = time.time()
    print("[EXP-C] rank_and_explain_node")
...
@observe(name="exp_c_answer_qa")
def answer_qa_node(state: AgentState):
    """후속 질문 답변."""
    print("[EXP-C] answer_qa_node")
...
if __name__ == "__main__":
    ...
    def run_test(case_id: str):
        ...
        session_id = f"exp_c_{uuid.uuid4().hex[:6]}"
