...
4. "General" 또는 "All_Domestic" 카테고리 혜택은 전 가맹점 적용 기본 혜택이므로, 유저의 전체 소비 금액({total_budget:,}원)에 대해 계산하라.
5. 위 4번을 제외하고, 유저 소비 카테고리와 무관한 혜택은 계산하지 말 것.
6. 데이터에 명확한 수치가 없는 혜택은 "unclear"로 분류하고 0원 처리.
7. 계산 과정(reasoning)을 반드시 포함하라. 어떤 근거로 어떤 공식을 적용했는지 기록.
